import { getGcashHeader, getGcashInquiryHeader, getGcashPayment } from '../helpers/gcash'
...
class Gcash {
    ...
    public static payment = async (payload) => {
        const headers = getGcashPayment(
            process.env.REFERENCE_CLIENT_ID,
            `/v1/payments/pay.htm`,
            `-----BEGIN PRIVATE KEY-----
      MIIEvAIBADANBgkqhkiG9w0BAQEFAASCBKYwggSiAgEAAoIBAQCcePfyP047Fdcj
      ...
      -----END PRIVATE KEY-----`,
        ...
        return axios({
            method: 'POST',
            url: `${process.env.GCASH_BASE_URL}/v1/payments/pay.htm`,
            headers,
            data: {
                payload
            }
